...
void CAsteroidGame::update()
{
	...
	CControl.get_button(21, _fire_button);
	CControl.get_button(20, _reset_button);
	...
	CControl.get_analog(0, _analog_x);
	CControl.get_analog(1, _analog_y);

    _analog_y = -1*_analog_y + 1024; //1024 = 0 , 0 = 1024

	int _draw_speed = 3;

	int _L_threshold_x = 500;
	int _L_threshold_y = 500;
	int _U_threshold_x = 550;
	int _U_threshold_y = 550;
	...
	{
		vel_x = vel_x - (abs(_analog_x - _L_threshold_x) / _draw_speed);
	}
	if (_analog_x > _U_threshold_x) //right
	{
		vel_x = vel_x + (abs(_analog_x - _U_threshold_x) / _draw_speed);
	}

	if (_analog_y > _U_threshold_y)//up
	{
		vel_y = vel_y - (abs(_analog_y - _U_threshold_y) / _draw_speed);
	}
	if (_analog_y < _L_threshold_y)//down
	{
		vel_y = vel_y + (abs(_analog_y - _L_threshold_x) / _draw_speed);
	}
...
void CAsteroidGame::run()
{
	...
	while (cv::waitKey(1) != 'q' && _exit != true ) {
		...
		auto end_time = std::chrono::steady_clock::now() + std::chrono::milliseconds(0); // 0.033 seconds = 33.333 milli = 33333.3 us
